...
func GenericTest(t *testing.T, part string, nclients int, unreliable bool, crash bool, partitions bool, maxraftstate int) {
	...
	for i := 0; i < 3; i++ {
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			j := 0
			defer func() {
				// log.Printf("client [%d]: quit!\n")
				clnts[cli] <- j
			}()
			last := ""
			key := strconv.Itoa(cli)
			Put(cfg, myck, key, last)
			// 这里还能使用函数外面的变量 done_client
		...
		DPrintf("5 Seconds later: tell clients to quit\n")
...
func TestSnapshotRPC3B(t *testing.T) {
	...
	DPrintf("1. [TEST] [TestSnapshotRPC3B] partition [0,1] [2]\n")
	...
	DPrintf("2.[TEST] [TestSnapshotRPC3B] partition [0,2] [1]\n")
	...
	// DPrintf("3.[TEST] [TestSnapshotRPC3B] partition [0,1, 2] \n")
	// now everybody
	...
	check(cfg, t, ck, "e", "E")
	check(cfg, t, ck, "1", "1")
	DPrintf("[TEST], [TestSnapshotRPC3B] done\n")
	cfg.end()
}
